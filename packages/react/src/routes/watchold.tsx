...
export default function Watch({
  id: idFromProps,
  miniPlayer: miniPlayerFromProps = false,
}: {
  id?: string;
  miniPlayer: boolean;
}) {
  ...
  const id = idFromProps || idFromParam;
      ...
      <div className="flex h-full w-full  @container" ref={ref}>
        ...
        >
          <div
            className={cn(
              "flex flex-col gap-4",
              miniPlayerFromProps ? "w-[500px]" : "w-full",
            )}
            // style={{ width: miniPlayerFromProps ? "600px" : undefined }}
          >
            <div
          ...
          {!theaterMode && !miniPlayerFromProps && (
        ...
        {!miniPlayerFromProps && data && bounds.width < 1023 && (
          <ChatModal
            tlOpen={tlOpen}
            chatOpen={chatOpen}
            id={data.id}
