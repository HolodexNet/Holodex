  ...
  >
    ...
    <div ref="watchLayout" class="flex flex-grow left">
      <div class="flex flex-col flex-grow">
        <div style="position: relative">
          ...
          <portal-target
            :name="`${video.id}-overlay`"
            style="font-size: 16px; font-size: max(1.5vw, 16px)"
          />
        </div>
        <WatchHighlights
    ...
    >
      <v-card elevation="5">
        <UploadScript
          :video-data="video"
          @close="$store.commit('setUploadPanel', false)"
        />
      </v-card>
    </v-dialog>
  </div>
...
export default {
  ...
  computed: {
    ...
    showHighlightsBar() {
      return (
        (this.comments.length || this.video.songcount) &&
        (!this.isMobile || !this.showTL)
      );
    },
    showUpload() {
...
.video {
  ...
  padding-bottom: min(
    56.25%,
    calc(
      100vh - #{$nav-bar-height} - #{$controls-height} - #{$title-info-height}
    )
  );
  width: 100%;
}
...
.watch-layout {
  ...
  @media (orientation: portrait) and (max-width: 959px) {
    ...
    .chat {
      ...
      height: calc(
        100% - #{$controls-height} - #{$nav-bar-height} - 100vw * 0.5625
      );
      /* iOS 11.2 and later */
      height: calc(
        (100% - #{$controls-height} - #{$nav-bar-height} - 100vw * 0.5625) -
    ...
    &.mobile .chat {
      ...
      /* iOS 11.2 and later */
      height: calc(
        (100% - #{$controls-height} - 100vw * 0.5625) -
          env(safe-area-inset-top, 0px)
      );
    }
    ...
    &.show-chat .left {
      ...
      padding-bottom: calc(
        (100vh - #{$controls-height} - 100vw * 0.5625) -
          env(safe-area-inset-top, 0px)
      );
    }
  }
