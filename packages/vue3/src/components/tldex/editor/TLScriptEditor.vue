<template>
  ...
  >
    <div class="d-flex flex-column" style="height: 100%; width: 100%">
      ...
      >
        <v-card ref="tableContainer" class="grow" height="100%" width="50%">
          <v-simple-table
    ...
    >
      ...
      <v-card v-if="modalMode === 6">
        <ExportFile
          :entries="entries"
          :profile="profile"
          :title="user.username + ' - ' + videoData.title"
        />
      </v-card>
      ...
      <v-card v-if="modalMode === 8">
        <v-container>
          ...
          <v-card-actions v-if="!editorMode">
            <v-btn
              color="primary"
              :disabled="!elevatedUser"
              @click="activateEditorMode()"
            >
              {{
                elevatedUser
                  ? $t("views.scriptEditor.editorModePanel.activateMode")
                  : $t("views.scriptEditor.editorModePanel.roleWarning")
              }}
            </v-btn>
          </v-card-actions>
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      offsetInput: 0 as string | number,
      // ------ SETTING ------
      TLLang: TL_LANGS[0],
      // ---- ACTIVE VIDEO ----
      activeURLInput: "",
      ...
      barCount: 0,
      displayEntry: 0,
      timecardIdx: [] as number[],
      resizeActive: false,
      resizeMode: 0, // some kind of resize switching? unsure.
      xPos: 0,
      // ---- EDITOR MODE ----
      editorMode: false,
      tlList: [],
  ...
  computed: {
    ...
    timerPrint() {
      const timeRaw = this.timerTime;
      return tlClientTimeFormat(timeRaw);
    },
    timeStampStart() {
      const timeRaw = this.entries[this.selectedEntry].Time;
      return tlClientTimeFormat(timeRaw);
    },
    timeStampEnd() {
      const timeRaw =
  ...
  methods: {
    ...
    addEntry() {
      ...
      if (!inserted) {
        if (this.entries.length !== 0) {
          this.entries[this.entries.length - 1].Duration =
            dt.Time - this.entries[this.entries.length - 1].Time;
    ...
    continuousTime() {
      ...
      for (let idx = 0; idx < this.entries.length - 1; idx += 1) {
        if (
          ...
          this.entries[idx].Duration =
            this.entries[idx + 1].Time - this.entries[idx].Time;
    ...
    processLog(forget?: boolean) {
      if (this.transactionLog.length > 0) {
        ...
        const processedLog: TransactionLogItem[] = [];
        logCopy.forEach((e) => {
          if (e.type === "Delete") {
            processedLog.push({
              type: "Delete",
        ...
        if (forget) {
          ...
          backendApi
            ...
            .then(({ status, data }) => {
              if (status === 200) {
                data.forEach(
                  (e: { type: string; tempid: any; res: { id: any } }) => {
                    if (e.type === "Add") {
    ...
    /**
     * Scrolls the script editor bar to the correct position
     * */
    scrollCalculator(): void {
      const deltaBar: number =
        this.timerTime / 1000 / this.secPerBar - this.barCount;
      ...
      this.$refs.TimelineDiv.scrollLeft =
        (this.timerTime / 1000 - this.barCount * this.secPerBar) * this.secToPx;
    },
    ...
    async filterTlChange(val: string) {
      let fetchChat = await (
        await backendApi.chatHistory(this.videoData.id, {
          lang: this.TLLang.value,
          verified: 0,
          moderator: 0,
          vtuber: 0,
      ...
      if (val !== "None") {
        fetchChat = fetchChat.filter((e: { name: any }) => e.name === val);
      }
    ...
    processImportData(eventData: any) {
    ...
    async settingOKClick() {
      ...
      if (parseVideoID) {
        const vidData = await (
          await backendApi.video(parseVideoID.id, this.TLLang.value)
        ).data;
        this.videoData = {
          id: parseVideoID.id,
    ...
    shiftTime() {
      const offset = Number.parseFloat(this.offsetInput as any);
