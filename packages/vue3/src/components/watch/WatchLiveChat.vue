<template>
  <v-sheet
    ...
    :class="{
      ...
      fluid: fluid,
      'mobile-live-chat': display.mobile,
    }"
  >
    ...
    <template v-if="canShowTLChat && showTlChat">
      <ArchiveTranslations
        ...
        :video="video"
        :class="{
          'stick-bottom': tlSettings.liveTlStickBottom,
    ...
    <template v-else-if="showTlChat">
      <v-card class="text-body-2 tl-overlay">
        <v-card-text
          :class="{
            'stick-bottom': $store.state.settings.liveTlStickBottom,
            'tl-full-height': !showYtChat,
          ...
          :style="{ height: tlChatHeight }"
        >
          This video is members only, please play the video to see TLdex and
          translations.
        </v-card-text>
      </v-card>
    </template>
...
export default defineComponent({
  ...
  components: {
    // LiveTranslations,
    ArchiveTranslations: () =>
      import("@/components/chat/ArchiveTranslations.vue"),
  },
  props: {
    video: {
      type: Object,
  ...
  emits: ["videoUpdate"],
  setup() {
    const tlSettings = useTLStore();
    const display = useDisplay();
    return { tlSettings, display };
  },
  data() {
    return {
      ...
      // @ts-ignore
      needExtension:
        !window.ARCHIVE_CHAT_OVERRIDE && this.video.status === "past",
    };
  },
  computed: {
    canShowTLChat() {
      return (
        (this.video.topic_id === "membersonly" && this.currentTime > 0) ||
        this.video.topic_id !== "membersonly"
      );
    },
    showTlChat() {
    ...
    ytChatHeight() {
      ...
      return this.tlSettings.liveTlWindowSize > 0 &&
        this.showTlChat &&
        !this.fixedBottom &&
        !this.fixedRight
        ? `${100 - this.tlSettings.liveTlWindowSize}%`
        : "";
    },
    tlChatHeight() {
      // Opposite of above
      return this.showYtChat && this.tlSettings.liveTlWindowSize > 0
        ? `${this.tlSettings.liveTlWindowSize}%`
        : "";
    },
    isArchived() {
