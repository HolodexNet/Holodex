<template>
  <v-card class="watch-card rounded-0">
    ...
    <v-card-subtitle>
      ...
      >
        •
        <router-link :to="searchTopicUrl" style="text-decoration: none">
    ...
    <div class="d-flex justify-space-between flex-wrap align-center">
      ...
      <v-col cols="auto">
        ...
        <template v-for="mention in channelChips" :key="mention.id">
          <ChannelChip :channel="mention" :size="60" />
        </template>
        <a
  ...
  computed: {
    formattedTime() {
      switch (this.video.status) {
        case "upcoming":
          return formatDistance(this.video.start_scheduled);
        case "live":
          return this.$t("component.watch.streamingFor", [this.elapsedTime]);
        default:
          return dayjs(this.video.available_at).format("LLL");
      }
    },
    liveViewers() {
      if (!this.video.live_viewers) return "";
      return formatCount(+this.video.live_viewers);
    },
    liveViewerChange() {
    ...
    processedMessage() {
      ...
      const vidUrl =
        (this.settings.redirectMode ? "https://youtu.be/" : "/watch/") +
        this.video.id;
