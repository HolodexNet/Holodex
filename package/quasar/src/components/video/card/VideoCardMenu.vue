<template>
  <v-menu v-model="showMenu" bottom :close-on-content-click="false" transition="slide-y-transition">
    <template #activator="{ props }">
      <label
        tabindex="0" class="m-1 border-none btn-outline btn-ghost btn btn-circle btn-sm" v-bind="props"
        @click.stop.prevent
      >
    ...
    <v-sheet>
      <ul class="p-2 overflow-y-auto menu bg-base-300 text-base-content menu-compact">
        <!-- Real Videos -->
        <template v-if="video.type !== 'placeholder'">
          <li>
            <a :href="`https://youtu.be/${video.id}`" target="_blank" @click.stop="closeMenu()">
              <v-icon left>
                {{ icons.mdiYoutube }}
              </v-icon>
          ...
          <li v-if="video.status === 'upcoming'">
            <a @click.prevent.stop="openGoogleCalendar(); closeMenu()">
              <v-icon left>
                {{ icons.mdiCalendar }}
              </v-icon>
          ...
          <li>
            <router-link :to="`/edit/video/${video.id}${video.type !== 'stream' ? '/mentions' : '/'}`">
              <v-icon left>
                {{ icons.mdiPencil }}
              </v-icon>
        ...
        <template v-else>
          <v-list-item
            ...
            @click.prevent.stop="openGoogleCalendar(); closeMenu()"
          >
            <v-icon left>
              {{ icons.mdiCalendar }}
            </v-icon>
        ...
        <li>
          <a>
            ...
            {{ ['live', 'upcoming'].includes(video.status) ? $t("component.videoCard.openClient") :
...
<script lang="ts">import { formatDuration } from "@/utils/time";
