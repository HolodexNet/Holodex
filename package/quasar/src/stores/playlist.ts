...
export const usePlaylistStore = defineStore("playlist", {
    ...
    actions: {
        ...
        reorder({ from, to }: { from: number; to: number }) {
            ...
            this.active.videos =
                this.active.videos.reduce((prev: VideoRef[], current, idx, self) => {
                    if (from === to) {
                        prev.push(current);
                    }
                    if (idx === from) {
        ...
        removeVideoByIndex(index: number) {
            this.active.videos =
                this.active.videos.filter((_, idx) => idx !== index)
            this.isSaved = false;

        },
        removeVideoByID(videoId: string) {
            this.active.videos =
                this.active.videos.filter((x) => x.id !== videoId)
            this.isSaved = false;
        },
        ...
        async saveActivePlaylist() {
            ...
            if (!this.active.id) {
                const res = await backendApi.savePlaylist(
                    { ...this.active, videos: [], video_ids: playlist.videos.map((x) => x.id) },
                    currentUserJWT,
                );
        ...
        async deleteActivePlaylist() {
            ...
            if (this.active.id && currentUserJWT && +this.active.user_id! === +currentUserId!) {
