...
export default {
  ...
  methods: {
    ...
    loadMessagesRepeating() {
      setInterval(async () => {
        const out = await fetch("http://localhost:5000/array");
        //  [{start, end, text}]
        const subs = await out.json();
        this.tlHistory = subs.map((x) => ({
          name: "whisper",
          is_tl: true,
          parsed: x.text,
          key: `${x.text}:${x.start}:${x.end}`,
    ...
    parseMessage(msg) {
      ...
      msg.relativeMs = this.startTimeMillis
        ? msg.timestamp - this.startTimeMillis
        : 0;
